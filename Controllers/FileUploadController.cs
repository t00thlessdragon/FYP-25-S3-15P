...
public async Task<IActionResult> UploadStaff(IFormFile staffFile)
{
    try
    {
        ...
        while (!reader.EndOfStream)
        {
            ...
            // Verify Program
            DbProgram? programEntity = null;
if (!string.IsNullOrWhiteSpace(programName))
{
    programEntity = await _db.Programs
        .FirstOrDefaultAsync(p => p.UniID == uniId && p.ProgramCode.ToUpper() == programName.ToUpper()); // Changed from p.ProgramName to p.ProgramCode
...
public async Task<IActionResult> UploadStudent(IFormFile studentFile)
{
    try
    {
        ...
        int uniId = admin.UniID.Value;
        int successCount = 0, skipCount = 0, lineNo = 0;
        // Collect detailed reasons for rows that were skipped and not processed
        ...
        while (!reader.EndOfStream)
        {
            ...
            if (cols.Length < 9)
            {
                var reason = $"Line {lineNo} ('{cols[0]}'): Missing columns ({cols.Length} found, 9 required).";
                _logger.LogWarning($"Skipped line {lineNo} ('{cols[0]}') - Missing columns. Line: {line}");
                skipDetails.Add(reason);
                skipCount++;
                continue;
            }
            ...
            {
                var reason = $"Line {lineNo} ('{studentId}'): Missing mandatory fields (ID, Name, or Email).";
                _logger.LogWarning($"Skipped line {lineNo} ('{studentId}') - Missing mandatory fields (ID, Name, or Email).");
                skipDetails.Add(reason);
                skipCount++;
                continue;
            }
            ...
            if (!string.IsNullOrWhiteSpace(uniNameCsv))
            {
                ...
                if (!string.Equals(adminUniName.Trim(), uniNameCsv.Trim(), StringComparison.OrdinalIgnoreCase))
                {
                    var reason = $"Line {lineNo} ('{studentId}'): University mismatch: CSV='{uniNameCsv}', Admin='{adminUniName}'.";
                    _logger.LogWarning($"Skipped line {lineNo} ('{studentId}') - University mismatch: CSV='{uniNameCsv}', Admin='{adminUniName}'.");
                    skipDetails.Add(reason);
                    skipCount++;
                    continue;
                }
            ...
            // --- 2. Program Lookup ---
            var programEntity = await _db.Programs
                .FirstOrDefaultAsync(p => p.UniID == uniId && p.ProgramCode.ToUpper() == programName.ToUpper().Trim());
            if (programEntity == null)
            {
                // LOGGING ADDED
                var reason = $"Line {lineNo} ('{studentId}'): Program Code not found: '{programName}'.";
                _logger.LogWarning($"Skipped line {lineNo} ('{studentId}') - Program Code not found: '{programName}'.");
                skipDetails.Add(reason);
                skipCount++;
                continue;
            }
            ...
            var courseEntity = await _db.Courses
                .FirstOrDefaultAsync(c => c.ProgramID == programEntity.ID && c.CourseCode.ToUpper() == courseName.ToUpper().Trim()); // Changed from c.CourseName to c.CourseCode
            if (courseEntity == null)
            {
                // LOGGING ADDED
                var reason = $"Line {lineNo} ('{studentId}'): Course Code not found for Program '{programName}': '{courseName}'.";
                _logger.LogWarning($"Skipped line {lineNo} ('{studentId}') - Course Code not found for Program '{programName}': '{courseName}'.");
                skipDetails.Add(reason);
                skipCount++;
                continue;
            }
            ...
            if (!string.IsNullOrWhiteSpace(sessionNameCsv))
            {
                ...
                {
                    // LOGGING ADDED
                    _logger.LogWarning($"Skipped line {lineNo} ('{studentId}') - Session not found: '{sessionNameCsv}'. (Non-critical)");
                    // This is non-critical, so we do NOT add it to skipDetails or increase skipCount
                }
            }
        ...
        return Json(new
        {
            ...
            skipDetails = skipDetails // <-- New: This array contains the detailed reasons for skipped rows.
        });
    }
    catch (Exception ex)
    {
        _logger.LogError(ex, "Error occurred during student file upload process.");
        // New: Return the specific exception message for easier debugging.
        return Json(new { success = false, message = $"An error occurred while uploading the student file: {ex.Message}. Please try again." });
    }
}
